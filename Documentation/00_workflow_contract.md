...
```
┌─────────────────────────────────────────────────────────────────────────┐
│ 1. TOUJOURS se baser sur MES VRAIS ORIGINAUX  →  Notebook_2018-2021/ipynb/   │
│    (régénérés via `bash scripts/restore_originaux.sh`)                    │
│    JAMAIS sur `main` ni sur 3_Sessions_Ratees/ (versions gutées à jeter). │
│                                                                           │
│ 2. S'INSPIRER de Notebook_2026/ (travail récent) UNIQUEMENT pour les 24      │
│    sujets NEUFS (DS_/DE_/AI_/MLE_/MLOps_ — ils n'ont pas d'original).     │
│    Pour tout notebook qui A un original, Notebook_2026/ n'est PAS une base.  │
└─────────────────────────────────────────────────────────────────────────┘
```

- **Refonte d'un de MES notebooks** → source = `Notebook_2018-2021/ipynb/<nom>.ipynb`. On préserve
  mes sections, on modernise, on teste. `Notebook_2026/` peut servir de simple référence d'idées,
  pas de point de départ.
- **Sujet NEUF (un des 24)** → pas d'original ; on part du plan (`Notebook_2026/plans/`) et on
  s'inspire du refait existant (`Notebook_2026/ipynb/`), à valider/tester intégralement.
...
1. **S'assurer d'avoir les vrais originaux** : `bash scripts/restore_originaux.sh` (dézippe `Notebook_2018-2021/Notebooks.zip` → `Notebook_2018-2021/ipynb/`). Le `.md` correspondant est dans `Notebook_2018-2021/md/<nom>.md`.
2. **Lecture du `md` original** : `Read` (pas `Grep`) le `Notebook_2018-2021/md/<nom>.md` **en entier** (le VRAI original, pas un guté). Si > 25k tokens, en plusieurs passes avec `offset`/`limit`. Lire tout le code, pas seulement les titres.
...
7. **Intégration du code dans le `.md`** : écrire `Notebook_2026/md/<nom>.md` en **copiant-collant** le code testé du sandbox + enrobant des cellules markdown selon les règles de format.
8. **Audit format** (pre + post conversion) :
   ```bash
   uv run python scripts/check_format.py --both Notebook_2026/md/<nom>.md Notebook_2026/ipynb/<nom>.ipynb
   ```
...
9. **Conversion `md` → `ipynb`** : `uv run jupytext --to ipynb --output Notebook_2026/ipynb/<nom>.ipynb Notebook_2026/md/<nom>.md`.
...
uv run python scripts/check_format.py --pre Notebook_2026/md/<nom>.md

# Post-conversion : audit du .ipynb
uv run python scripts/check_format.py --post Notebook_2026/ipynb/<nom>.ipynb
...
- Les 44 fichiers dans `Notebook_2026/ipynb/` ont presque tous été bâtis sur des **gutés** :
...
- Les vrais originaux sont dans `Notebook_2018-2021/` (zip committé + `ipynb/` gitignoré régénérable
...
Le `.venv` et le kernel sont partagés (côté WSL) ; seul le code/markdown du notebook est isolé. La source des originaux (`Notebook_2018-2021/Notebooks.zip` + `md/`) est committée donc présente dans chaque worktree.
...
- Toujours committer SES fichiers par chemin explicite, p. ex. :
  `git commit -m "…" -- Notebook_2026/ipynb/<nom>.ipynb Notebook_2026/md/<nom>.md Notebook_2026/_doc/00_FAIT_A_FAIRE.md`
