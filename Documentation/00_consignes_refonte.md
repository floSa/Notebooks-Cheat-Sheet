...
1. `bash scripts/restore_originaux.sh` = **dézippe mes originaux** (`Notebook_2018-2021/Notebooks.zip`
   → `Notebook_2018-2021/ipynb/`). Ce ne sont PAS dans git directement (trop lourds), d'où le dézip.
2. **`ipynb → md`** : la version markdown lisible de l'original existe déjà dans
   `Notebook_2018-2021/md/<NOM>.md` (générée par **jupytext**). Si elle manque :
   `uv run jupytext --to md Notebook_2018-2021/ipynb/<NOM>.ipynb`.
   (jupytext récupère TOUTES les cellules — markdown ET code.)
3. **Étudier** ce `.md` original en entier.
4. **Pareil pour la refonte déjà faite** si elle existe : lire `Notebook_2026/md/<NOM>.md`.
5. **Réfléchir au plan en repartant de l'existant** (cf. §1 et §2).

## 1. Les sources — dans cet ordre

**(A) BASE PRINCIPALE = mon vrai original** : `Notebook_2018-2021/ipynb/<NOM>.ipynb`
   (version lisible : `Notebook_2018-2021/md/<NOM>.md`).
...
**(B) SOURCE SECONDAIRE = la refonte déjà faite**, SI elle existe :
   `Notebook_2026/ipynb/<NOM>.ipynb` (+ `Notebook_2026/md/<NOM>.md`).
...
- `Notebook_2026/md/<NOM>.md` puis conversion → `Notebook_2026/ipynb/<NOM>.ipynb`.
...
```
Les consignes à suivre scrupuleusement sont dans ce document : Documentation/00_consignes_refonte.md (lis-le en entier d'abord).
