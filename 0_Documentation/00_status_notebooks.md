...
| 2 | `Structures_L_T_D_Cheat_Sheet.md` | ✅ fait | CS + WIKI | L | | Refait sur le vrai original (599 l. lisibles, 52 cellules, **7 images base64 diagrammes**). 5 critères contrat OK : sandbox exit 0, `check_format --both` vert. 31 cellules code, 9 sections, **7 images 100 % préservées** (array 1D, reshape C/F, stack, 3D-from-2D, list↔tuples). **Original préservé & corrigé** : bugs `ma_Sttring`→`ma_String`, `flatten`→`flatten_obj`, indentation merge, bloc xarray qui plantait (`ds.mean(dim=['species'])`/`sel(sepal_length=slice)`) refait avec démo qui tourne. **Ajouts 2026** : Tuples (unpacking/dataclass frozen), Sets, `collections` (Counter/defaultdict/deque/OrderedDict/ChainMap), NumPy indexing avancé + broadcasting (LaTeX) + ufuncs, xarray DataArray labellisé exécuté + groupby + NetCDF. Libs ajoutées : xarray, netcdf4, iteration_utilities. |
| 3 | `Structures_DataFrame.md` | ✅ fait | TUTO + CS + WIKI | L | 🌐 | Refait sur le vrai original (1624 l, 131 cellules, **9 images**). 5 critères contrat OK : sandbox exit 0, `check_format --both` vert. 57 cellules code, 17 sections. **Original 100 % préservé + corrigé** (API mortes `df.append`/`df.lookup`/`pd.lreshape`/`np.NaN`/`fillna(method=)`/google.colab remplacées). **Ajouts 2026** : vectorisation, groupby `agg` nommé, merge_asof, anti-join/upsert typés, crosstab, pandas 3.0 (str dtype/CoW/Arrow), Polars lazy, formules LaTeX (σ/SEM/quantiles). |
| 4 | `Structure_BDD_&_DataFrame.md` → `Structure_BDD_DataFrame.md` | ✅ fait | TUTO + CS | M | 🌐 | Refait sur le vrai original (`1_Old_Notebooks/md/Structure_BDD_&_DataFrame.md`). 19 cellules code, 50 markdown. Sandbox end-to-end exit 0 + `check_format --both` vert. **Original 100 % préservé & corrigé** : write/read × {Postgres SQLAlchemy, Postgres psycopg, MongoDB pymongo} — bug virgule `to_sql` corrigé, `psycopg2`→**psycopg 3**. **Ajouts** : panorama comparatif, sécurité (pydantic-settings/env vars, masquage, anti-injection), démos exécutables SQLite + **mongomock** (round-trip Titanic), backends 2026 (**ADBC** `dtype_backend=pyarrow`, **Polars/connectorx**, **DuckDB** zero-copy + Parquet), bulk **COPY**, **upsert** ON CONFLICT, **benchmark** chiffré + graphique, streaming `chunksize`, **retry** tenacity. Libs ajoutées : pymongo, mongomock, connectorx, adbc-driver-sqlite/manager. |
...
| 11 | `Detection_Outliers.md` (renommé) | ✅ fait (conforme contrat 5 critères) | WIKI + TUTO | M | 🌐 | **Refonte complète selon workflow, basée sur le VRAI original** (`1_Old_Notebooks/md/Détection D_outliers.md`, 480 lignes lues intégralement) : plan + structure en sandbox, sandbox `.py` exécuté end-to-end (exit 0), `check_format --both` vert (**87 cellules / 25 code**). Modernisation 2026 réutilisée de la refonte existante. **Datasets** : synthétique 2D **avec ground-truth** (make_blobs + outliers injectés) + Titanic (mutualisé) + Air Passengers. **5 bugs de l'original fixés** (X=iris écrasé, OCSVM 1-point, drop index incohérent, distplot déprécié, glass.csv perdu). **3 images base64 de l'original PRÉSERVÉES** (réextraites en PNG dans `ipynb/images/` : intuition Elliptic Envelope, schéma IQR/boxplot, formule distance de Cook) + viz **boxplot avant/après suppression réintégrée (§3.5)**. **Supprimés (justifiés, non graphiques)** : Colab mount, df.head inliers, pseudo-code final cassé (melt/pivot/dfa.merge non exécutable). **Ajouts** (rôle WIKI 2026) : cadrage outlier/novelty/anomaly, Z-score+MAD (maths), maths Mahalanobis/LOF/IForest, comparatif chiffré ROC-AUC + precision@k, autoencoder PyTorch, **PyOD ECOD/COPOD param-free exécutés**, matrix profile STUMPY, guide de choix, évaluation/seuil. Comparatif mesuré : Elliptic AUC=0.999, IForest=0.998, OCSVM=0.998, LOF=0.996, ECOD=0.988. |
...
| 13 | `ML_Regression_Classification_CV_GridSearch.md` | ✅ fait (conforme contrat 5 critères) | WIKI + CS | L | 🌐 | **Refonte complète sur le VRAI original** (`1_Old_Notebooks/ipynb/...`, **123 cellules** lues intégralement — ⚠️ le plan précédent avait lu une version gutée à 16 cellules). Plan + structure dans `scripts/_sandbox/`. Sandbox `.py` exécuté end-to-end (exit 0, résultats cohérents). `check_format --both` vert (**178 cellules / 67 code / 111 md**). **2 images base64 de l'original PRÉSERVÉES** (schéma K-fold + comparatif classifieurs sklearn). **Garde** tout le walkthrough par modèle (baseline→cross_validate→GridSearchCV avec lecture `cv_results_` par param ET par fold) : régression (Lasso, DT, SGD, SVR, RF, GB, AdaBoost), clustering (KMeans/DBSCAN), classification (KNN, DT, NB, RF, SGD, SVC, GB, AdaBoost, MLP), hyperopt + Optuna. **Bugs originaux corrigés** : `pip install optuna` (non-Python → markdown), `suggest_uniform` déprécié → `suggest_float`, KMeans (`silhouette_score`/`adjusted_rand_score` non importés, réassignation écrasant la fonction, `X.reshape(1,-1)` cassant la CV, `algorithm='auto'` retiré en sklearn 1.8 → ARI scorer + elbow), DBSCAN (`dbscan.score()` inexistant). **Supprimé** (justifié, non graphique) : 2 cellules vides. **Ajouté** (greffe de la refonte gutée + maths) : théorie CV (erreur de généralisation, biais-variance, table splitters), stratégies de recherche (Grid/Random/Halving/Bayes), Pipeline+ColumnTransformer anti-leak, bench multi-modèles réel (California Housing + Titanic) + RandomizedSearchCV/HalvingRandomSearchCV, applications réelles hyperopt/Optuna (tuning RF), calibration des probabilités (Brier + courbe), pièges, sources. Dep ajoutée : `hyperopt 0.2.7`. Datasets : make_regression/make_blobs (walkthrough) + California Housing + Titanic (bench réel). |
...
| 15 | `ML_Optimisation_de_Modèles.md` | ✅ fait (conforme contrat 5 critères) | TUTO + WIKI | M | 🌐 | **Refonte complète selon workflow** : original lu intégralement (598 l.) ; plan + structure dans `scripts/_sandbox/`. Sandbox `.py` exécuté end-to-end (exit 0) : sklearn Grid/Random/Halving (R² 0.85/0.85/0.54), Optuna toy `(x-2)²` (x≈2.0), espace conditionnel Ridge/RF/GBR (R²=0.85), samplers, **pruner exécuté** (SGD `partial_fit` + `MedianPruner`, 10 complétés/10 élagués, split val interne), **XGB/LGBM/CatBoost** (R² 0.864/0.866/0.861), visualisations (history/importances/slice), **multi-objectif NSGA-II** + front de Pareto (4 sol.). `check_format --both` vert (70 cellules / 18 code / 52 md). **Mutualisé California Housing** (régression). **2026** : GPSampler/AutoSampler, NSGA-II/III, migration `optuna-integration`, Optuna vs Ray Tune (ASHA) vs KerasTuner. **Bugs originaux corrigés** : `pip install` en cellules (→ uv), cellule CatBoost indentée cassée, sur-souscription OpenMP (`n_jobs=1` estimateurs). **Supprimé** : exemples classification iris/breast_cancer/MNIST (→ régression California Housing), boucle torch non exécutée, exemple Keras MNIST lourd (décision user : pas de DL exécuté), snippets définis-mais-jamais-exécutés. **Ajouté** : vocabulaire HP/param, espaces & distributions, Grid/Random/Halving sklearn, théorie bayésienne, pruner exécuté, visualisations Optuna, multi-objectif réel, tableau par framework, paysage outils 2026, pièges. Datasets : California Housing. |
| 16 | `ML_Explication_Feature_Importance_Selection.md` | ✅ fait (conforme contrat 5 critères) | WIKI + TUTO + CS | L | 🌐 | **Refonte complète selon workflow** : original lu intégralement (2468 l. — carnet fourre-tout ~9 datasets dont la plupart perdus sur Drive), plan + structure dans `scripts/_sandbox/`. Sandbox `.py` exécuté end-to-end (exit 0, sanity checks OK : `noise_random` classé dernier en \|SHAP\|, `MedInc` top-3, Boruta ne rejette que le bruit), `check_format --both` vert (113 cellules / 38 code / 75 md). **Recentré sur California Housing** (mono-dataset → testable, vs original non exécutable). **Garde** (porté + refactoré en fonctions typées) : sélection RFECV/SelectFromModel/RF-MDI, permutation (réécrit eli5→sklearn), Boruta (démo shadow-features maison, BorutaPy cassé numpy 2), SHAP global/local/dépendance/interaction/redondance (API moderne), LIME. **Supprime** : galerie viz sklearn (ROC/Calibration/etc. hors-sujet), SHAP deep-learning CIFAR/Keras (lourd, non testable), zoo multi-datasets (Titanic/Iris/Abalone/Mushrooms/NHANES/Wine/Adult, perdus), cellules Colab + API SHAP legacy (summary_plot/force_plot/initjs) + bugs (boxplot `labels=`, `max_depht` typo). **Ajoute** : 2 features pièges (vérité terrain), démo biais cardinalité MDI, drop-column, mutual info, coefs L1/L2 (chemin Lasso), encadré importance≠causalité, synthèse 6 classements + Spearman. Dep ajoutée : `lime`. |
...
| 18 | `ML_Apprentissage_par_Renforcement.md` | ✅ fait (conforme contrat 5 critères) | WIKI + TUTO | L | 🌐 | **Refonte from scratch selon workflow** : original (187 lignes, 3 bandits sur CSV Google-Drive perdu) lu intégralement, plan + structure dans `scripts/_sandbox/`, sandbox `.py` exécuté end-to-end (exit 0 : Thompson regret 56 < ε-greedy 94 < UCB1 373 < aléatoire 527 ; FrozenLake succès ~0.95 ; DQN 24→129 ; PPO 500), `check_format.py --both` vert (72 cellules / 19 code). **Recadré CTR → RL général**. Bandits **gardés+corrigés** (off-by-one, hack `N*10`→inf, CSV→bandit Bernoulli synthétique à CTR connus pour mesurer le regret) + **ajout ε-greedy**. **Ajouts** : cadre MDP/Bellman (maths), value iteration sur GridWorld, Q-learning & SARSA (Gymnasium FrozenLake), DQN PyTorch (CartPole), SB3 PPO exécuté, panorama 2026 (Gymnasium/SB3/CleanRL/RLlib + RLHF→DPO→GRPO→RLVR, web-sourcé). Datasets 100% programmatiques. Deps ajoutées : `gymnasium`, `stable-baselines3`. |
...
| 34 | `TS_Time_Series_Overview.md` | ✅ fait (conforme contrat 5 critères) | WIKI | L | 🌐 | **Refonte complète selon workflow** : original lu intégralement (20 cellules), plan + structure dans `scripts/_sandbox/`. Sandbox `.py` exécuté end-to-end (exit 0) **et** `.ipynb` exécuté via nbconvert (exit 0, 0 cellule en erreur, 6 figures), `check_format --both` vert (75 cellules / 14 code / 61 md). **Cœur 100 % exécuté** (14 cellules) : baselines naïves, ETS (saison mul), SARIMA manuel + **AutoARIMA** (pmdarima), **Theta** (statsmodels natif), RF+XGB lag features, **modèle global** (1 XGB / 20 séries panel synthétique, WAPE 0.039), **bench unifié Nixtla `statsforecast`** (AutoARIMA/ETS/Theta/SeasonalNaive + `utilsforecast.evaluate`), **vrai LSTM PyTorch** (remplace la string pseudo-code de l'original), backtest **TimeSeriesSplit** expanding 5 folds, **conformal prediction** split (couverture 92 % / 90 % visé), comparatif multi-métriques (RMSE/MAE/sMAPE/**MASE**/**WAPE**) → ETS meilleur (MASE 0.338), 8/11 < naïf. **Dataset** : Air Passengers réel (`get_rdataset`, remplace le synthétique de l'original) + panel synthétique déterministe. **Décisions env** : lightgbm/mlforecast écartés (`libgomp.so.1` absent) → xgboost ; statsforecast 2.0.3 installé. **Prose 2026 (recherche web)** : matrice de décision, panorama archis DL (PatchTST/iTransformer/TSMixer + nuance GIFT-Eval), **foundation models** Chronos-2 (oct 2025)/TimesFM 2.5/Moirai 2.0/TimeGPT, conformal, stack, pièges. **Supprimé** : string pseudo-code LSTM (anti-pattern), série synthétique, Prophet exécuté (build cmdstan fragile → décrit en prose). Dep ajoutée : `statsforecast`. |
| 35 | `TS_ARIMA.md` (renommé) | ✅ fait (conforme contrat 5 critères) | WIKI + TUTO + CS | M | 🌐 | **Refonte complète selon workflow** : original lu intégralement (21 cellules), plan + structure dans `scripts/_sandbox/`. Sandbox `.py` exécuté end-to-end (exit 0) **et** `.ipynb` exécuté via nbconvert (exit 0, figures intégrées), `check_format --both` vert (46 cellules / 9 code / 37 md). **13 sections** (vs 10 original). **Cœur exécuté** : famille AR/MA/ARMA/ARIMA/SARIMA/SARIMAX (équations LaTeX), Box-Jenkins, **ADF + KPSS** (tests complémentaires) + **décomposition saisonnière** (ajout), lecture ACF/PACF + table, **AutoARIMA pmdarima `(1,1,0)(0,1,0,12)` + statsforecast** (ordre concordant), fit SARIMA (**MAPE 4.18 %**), forecast + IC 95 %, **diagnostic résidus** (Ljung-Box imprimé p>0.05 / Q-Q / ACF), **SARIMAX exogène exécutable** (exog jouet → RMSE inchangé = leçon causalité). **Dataset** : Air Passengers réel (`get_rdataset`, **remplace le synthétique** de l'original — justif : montre la saisonnalité multiplicative). **Prose 2026 (recherche web)** : pmdarima 2.1.1 maintenu, statsforecast recommandé, foundation models TimesFM 2.5/Chronos-2/Moirai-2/TimeGPT. **Conservé** : table notation, anti-patterns (restaurée vs v0), ARIMAX exécutable (restauré vs v0 qui l'avait réduit en pseudo-code). **Ajouté** : KPSS+décompo, statsforecast exécutable, fonctions typées+docstrings (adf_test/kpss_test/load_air_passengers), §12 foundation models. **Supprimé** : série synthétique. **Corrigé vs v0** : code SARIMAX coincé dans `<!-- #region -->` (pathologies #1/#4), pseudo-code commenté (#6), code suivi directement d'un titre. |
| 36 | `TS_Generer_Sequence.md` | ✅ fait | CS+TUTO | S | | Refonte contrat : 9 cellules code testées end-to-end (exit 0), `check_format --both` vert. NumPy (boucle + vectorisé stride_tricks), multi-features 3D, multi-output, PyTorch Dataset streaming, split temporel avec gap, scaling sans leakage, multi-séries (panel exécuté), démo réelle Air Passengers, outils 2026 (Nixtla/darts/pytorch-forecasting) + équivalents natifs Keras. |
| 37 | `TS_Maintenance_Prédictive.md` | 🗑️ supprimé | — | — | | Remplacé par version `_GOOD` renommée. |
| 38 | `TS_Maintenance_Predictive.md` (renommé) | ✅ fait (conforme contrat 5 critères) | CS + TUTO + WIKI | L | 🌐 | **Refonte complète selon workflow** : original `_GOOD` lu intégralement (8 cellules), plan + structure dans `scripts/_sandbox/`. Sandbox `.py` exécuté end-to-end (exit 0), `check_format --both` **vert** (59 cellules / 16 code / 43 md). **Passé au VRAI dataset NASA C-MAPSS FD001** (datasets.md l'exige) : download mirror GitHub + cache `data/_shared/turbofan/` + README + **fallback synthétique offline**. 100 moteurs train run-to-failure / 100 test tronqués + `RUL_FD001`. **Cœur exécuté** : RUL **piecewise-linear + clipping 125** (maths), EDA (durée de vie, trajectoires, **6 capteurs constants droppés par VarianceThreshold** → 15 informatifs, signature dégradation), feature eng rolling mean/std/**slope forme fermée raw=True**, **protocole d'éval officiel** (dernier cycle vs RUL file), **bench 7 modèles** (CatBoost MAE 12.5 best / GBM / XGBoost / RF / MLP / Lin / Ridge), **score asymétrique C-MAPSS** (CatBoost 794 vs baseline 33k), viz pred-vs-true + trajectoire, classif binaire RUL<30 (acc 0.91), **vrai LSTM PyTorch** (cible normalisée /cap → MAE 11.5, R² 0.83, bat le GBM). **Décisions env** : **lightgbm retiré du bench** (`libgomp.so.1` absent → fit crash, vérifié), catboost OK, torch CPU → LSTM allégé (1 couche, step=5, 15 ep, ~2 min). **Supprimé** : données simulées comme source principale (gardées en fallback only) ; PCA 3 stratégies du v0 (remplacé par sélection variance, plus pertinent FD001) ; pseudo-code LSTM du v0 (remplacé par vrai LSTM exécuté). **Recherche web 2026** : vérif source C-MAPSS (mirror edwardzjl fiable), modèles SOTA cités (TFT/N-BEATS/PatchTST/DeepSurv). |
...
- **Notebooks ✅ fait au sens du contrat (5 critères vérifiés)** : **15** — EDA_Visualisation_Introduction (#8), EDA_Analyse_Multivarie (#9), EDA_Stats_Analyse_Desc_Visual (#10), Detection_Outliers (#11), ML_Explication_Feature_Importance_Selection (#16), DL_KAN_Kolmogorov_Arnold (#26), TS_Time_Series_Intro (#33), TS_Time_Series_Overview (#34), TS_ARIMA (#35), TS_Generer_Sequence (#36), TS_Maintenance_Predictive (#38), BDD_Vectorielles (#40), DE_Docling (#43), DL_Keras (#46), DL_TensorFlow (#47).
- **🟡 v0** (squelette posé, code non vérifié end-to-end) : **31** — les autres anciennes "✅ fait".
...
- **24 plans détaillés** dans `2_New_Notebooks/plans/` couvrant DS / DE / AIE / MLE / MLOps
