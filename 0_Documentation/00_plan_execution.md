...
- `01_unzip.sh` — dézippe `Notebooks_ok/Notebooks.zip` vers `1_Old_Notebooks/ipynb/`.
- `02_ipynb_to_md.sh` — convertit tout `1_Old_Notebooks/ipynb/*.ipynb` vers `1_Old_Notebooks/md/*.md` via `jupytext --to md`.
- `04_md_to_ipynb.sh` — convertit tout `2_New_Notebooks/md/*.md` vers `2_New_Notebooks/ipynb/*.ipynb` via `jupytext --to ipynb`.
...
- Exécution de `01_unzip.sh` → 45 fichiers dans `1_Old_Notebooks/ipynb/`.
- Vérification : nom étrange `DL_PyTorch` (sans extension `.ipynb`) dans la liste du zip → inspecter (probablement un dossier).
- Exécution de `02_ipynb_to_md.sh` → 45 `.md` dans `1_Old_Notebooks/md/`.
...
1. **Lire** `1_Old_Notebooks/md/<notebook>.md` en entier.
...
3. **Brouiller** un plan de refonte dans la tête (ou en commentaire en haut du nouveau `.md`) :
   ...
   - Maths à intégrer
4. **Écrire** `2_New_Notebooks/md/<notebook>.md` en suivant strictement le format du `00_consignes.md`.
...
- Exécution finale de `04_md_to_ipynb.sh` sur tout `2_New_Notebooks/md/`.
- Inspection : ouvrir 2-3 notebooks aléatoires dans Jupyter pour vérifier le rendu.
- Vérification round-trip : `2_New_Notebooks/ipynb/<x>.ipynb` → re-convertir en `.md` → diff avec `2_New_Notebooks/md/<x>.md` (doit être nul ou cosmétique).
