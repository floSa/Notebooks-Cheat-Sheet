...
```
┌─────────────────────────────────────────────────────────────────────────┐
│ 1. TOUJOURS se baser sur MES VRAIS ORIGINAUX  →  1_Old_Notebooks/ipynb/   │
│    (régénérés via `bash scripts/restore_originaux.sh`)                    │
│    JAMAIS sur `main` ni sur 3_Sessions_Ratees/ (versions gutées à jeter). │
│                                                                           │
│ 2. S'INSPIRER de 2_New_Notebooks/ (travail récent) UNIQUEMENT pour les 24      │
│    sujets NEUFS (DS_/DE_/AI_/MLE_/MLOps_ — ils n'ont pas d'original).     │
│    Pour tout notebook qui A un original, 2_New_Notebooks/ n'est PAS une base.  │
└─────────────────────────────────────────────────────────────────────────┘
```

- **Refonte d'un de MES notebooks** → source = `1_Old_Notebooks/ipynb/<nom>.ipynb`. On préserve
  mes sections, on modernise, on teste. `2_New_Notebooks/` peut servir de simple référence d'idées,
  pas de point de départ.
- **Sujet NEUF (un des 24)** → pas d'original ; on part du plan (`2_New_Notebooks/plans/`) et on
  s'inspire du refait existant (`2_New_Notebooks/ipynb/`), à valider/tester intégralement.
...
1. **S'assurer d'avoir les vrais originaux** : `bash scripts/restore_originaux.sh` (dézippe `1_Old_Notebooks/Notebooks.zip` → `1_Old_Notebooks/ipynb/`). Le `.md` correspondant est dans `1_Old_Notebooks/md/<nom>.md`.
2. **Lecture du `md` original** : `Read` (pas `Grep`) le `1_Old_Notebooks/md/<nom>.md` **en entier** (le VRAI original, pas un guté). Si > 25k tokens, en plusieurs passes avec `offset`/`limit`. Lire tout le code, pas seulement les titres.
...
7. **Intégration du code dans le `.md`** : écrire `2_New_Notebooks/md/<nom>.md` en **copiant-collant** le code testé du sandbox + enrobant des cellules markdown selon les règles de format.
8. **Audit format** (pre + post conversion) :
   ```bash
   uv run python scripts/check_format.py --both 2_New_Notebooks/md/<nom>.md 2_New_Notebooks/ipynb/<nom>.ipynb
   ```
...
9. **Conversion `md` → `ipynb`** : `uv run jupytext --to ipynb --output 2_New_Notebooks/ipynb/<nom>.ipynb 2_New_Notebooks/md/<nom>.md`.
...
uv run python scripts/check_format.py --pre 2_New_Notebooks/md/<nom>.md

# Post-conversion : audit du .ipynb
uv run python scripts/check_format.py --post 2_New_Notebooks/ipynb/<nom>.ipynb
...
- Les 44 fichiers dans `2_New_Notebooks/ipynb/` ont presque tous été bâtis sur des **gutés** :
...
- Les vrais originaux sont dans `1_Old_Notebooks/` (zip committé + `ipynb/` gitignoré régénérable
...
Le `.venv` et le kernel sont partagés (côté WSL) ; seul le code/markdown du notebook est isolé. La source des originaux (`1_Old_Notebooks/Notebooks.zip` + `md/`) est committée donc présente dans chaque worktree.
...
- Toujours committer SES fichiers par chemin explicite, p. ex. :
  `git commit -m "…" -- 2_New_Notebooks/ipynb/<nom>.ipynb 2_New_Notebooks/md/<nom>.md 0_Documentation/00_FAIT_A_FAIRE.md`
