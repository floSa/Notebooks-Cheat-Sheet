...
- `1_Old_Notebooks/ipynb/DL_PyTorch.ipynb` (143 cellules) → **squelette** (intro, tenseurs, GPU,
  XOR, Dataset/DataLoader, loss, train, eval, save/load, régression, classification, ROC/AUC).
- `1_Old_Notebooks/ipynb/DL_Tensorflow_Keras.ipynb` (112 cellules) → **techniques avancées**
  (batch manuel équilibré/trié, `class_weight`, `sample_weight`, SHAP DeepExplainer).

Squelette commun imposé (16 sections) : `0_Documentation/00_blueprint_DL_frameworks.md`.
S'il existe déjà une refonte dans `2_New_Notebooks/ipynb/DL_<Framework>.ipynb`, **s'appuyer dessus**
...
```
Les consignes à suivre scrupuleusement sont dans ce document : 0_Documentation/00_consignes_DL.md (lis-le en entier d'abord).
