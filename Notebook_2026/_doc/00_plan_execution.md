...
- `01_unzip.sh` — dézippe `Notebooks_ok/Notebooks.zip` vers `Notebook_2018-2021/ipynb/`.
- `02_ipynb_to_md.sh` — convertit tout `Notebook_2018-2021/ipynb/*.ipynb` vers `Notebook_2018-2021/md/*.md` via `jupytext --to md`.
- `04_md_to_ipynb.sh` — convertit tout `Notebook_2026/md/*.md` vers `Notebook_2026/ipynb/*.ipynb` via `jupytext --to ipynb`.
...
- Exécution de `01_unzip.sh` → 45 fichiers dans `Notebook_2018-2021/ipynb/`.
- Vérification : nom étrange `DL_PyTorch` (sans extension `.ipynb`) dans la liste du zip → inspecter (probablement un dossier).
- Exécution de `02_ipynb_to_md.sh` → 45 `.md` dans `Notebook_2018-2021/md/`.
...
1. **Lire** `Notebook_2018-2021/md/<notebook>.md` en entier.
...
3. **Brouiller** un plan de refonte dans la tête (ou en commentaire en haut du nouveau `.md`) :
   ...
   - Maths à intégrer
4. **Écrire** `Notebook_2026/md/<notebook>.md` en suivant strictement le format du `00_consignes.md`.
...
- Exécution finale de `04_md_to_ipynb.sh` sur tout `Notebook_2026/md/`.
- Inspection : ouvrir 2-3 notebooks aléatoires dans Jupyter pour vérifier le rendu.
- Vérification round-trip : `Notebook_2026/ipynb/<x>.ipynb` → re-convertir en `.md` → diff avec `Notebook_2026/md/<x>.md` (doit être nul ou cosmétique).
