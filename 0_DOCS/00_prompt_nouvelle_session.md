...
AVANT TOUT : lis le contrat `0_DOCS/00_workflow_contract.md` (section 0 = RÈGLE D'OR).
Il définit le workflow obligatoire en 9 étapes, les 5 critères de "fait", le rapporting honnête.

🎯 RÈGLE D'OR :
- Si le notebook a un ORIGINAL (dans 1_MES_NOTEBOOKS/ipynb/) → je me base PRINCIPALEMENT
...
- Si c'est un SUJET NEUF (un des 24 : DS_/DE_/AI_/MLE_/MLOps_, pas d'original) → je pars du
  plan (2_NOUVEAUX/plans/) et je m'inspire du refait existant (2_NOUVEAUX/ipynb/).
- 2_NOUVEAUX/ n'est une base QUE pour les 24 sujets neufs, jamais pour un notebook qui a un original.

D'abord : bash scripts/restore_originaux.sh (régénère 1_MES_NOTEBOOKS/ipynb/ depuis le zip).

Notebook à refaire cette session : <NOM_DU_NOTEBOOK>

Applique le workflow 9 étapes en entier, sans raccourci :
1. S'assurer que 1_MES_NOTEBOOKS/ipynb/ existe (sinon restore_originaux.sh)
2. Read (PAS Grep) le 1_MES_NOTEBOOKS/md/<NOM>.md EN ENTIER (le VRAI original)
   — OU, si sujet neuf : 2_NOUVEAUX/plans/<NOM>.md + 2_NOUVEAUX/ipynb/<NOM>.ipynb
...
7. Intègre le code testé dans 2_NOUVEAUX/md/<NOM>.md (titre seul, description avant ET après code)
8. Audit : uv run python scripts/check_format.py --both 2_NOUVEAUX/md/<NOM>.md 2_NOUVEAUX/ipynb/<NOM>.ipynb -> doit être [OK] TOUT VERT
